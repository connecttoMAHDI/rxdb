    ...
    persistenceQueue = persistenceQueue.then(() => {
      ...
      return Promise.all([state.input.forkInstance.findDocumentsById(docIds, true), (0, _metaInstance.getAssumedMasterState)(state, docIds)]).then(([currentForkStateList, assumedMasterState]) => {
        ...
        return Promise.all(docIds.map(async docId => {
          ...
          if ((forkStateFullDoc && assumedMaster && isAssumedMasterEqualToForkState === false || forkStateFullDoc && !assumedMaster) && !state.skipStoringPullMeta && !(forkStateFullDoc._meta.o && forkStateFullDoc._meta.o.hash === identifierHash && forkStateFullDoc._meta.o._rev === (0, _index.getHeightOfRevision)(forkStateFullDoc._rev))) {
            /**
          ...
          newForkState._meta.o = {
            _rev: !forkStateFullDoc ? 1 : (0, _index.getHeightOfRevision)(forkStateFullDoc._rev) + 1,
            hash: identifierHash
          };
          var forkWriteRow = {
